...
class ExtractAssetBundle : ScriptableWizard
{
    ...
    [MenuItem("Assets/Bundles/Extract Remote AssetBundle (WIP)", false, 123)]
    static void CreateWizard()
    {
        ScriptableWizard.DisplayWizard("Extract AssetBundle", typeof(ExtractAssetBundle), "Download AssetBundle");
    }
    ...
    void OnGUI()
    {
        ...
        if (null == m_www)
        {
            if (GUI.Button(new Rect(10, 75, 300, 20), "Extract AssetBundle"))
            {
                Init();
                m_www =
